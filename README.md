...
| 大会 | ドメイン | 文体 |
|------|----------|------|
| 関西選手権 | `karal.jp` | 1000m/2000mタイム、`(n/m)`＋着順、【N日目のレース結果】形式 |
| インカレ（全日本大学選手権） | `jara.or.jp` | 500m/2000mタイム、`(n/m)`＋着順、「種目 予選N着→ 進出先へ」サマリー形式 |
...
3. **順位算出** (`nurc_gen/ranking.py`): 詳細欄はタイムの後ろに `(n/m) 4着` を付ける。
   行頭の番号はレーン番号なので、組内着順（`4着`）を別途明示する。`(n/m)` はソース
   HTMLに無い手計算値で、各ラウンド（予選・準々決勝・準決勝・CR 等）ごとに、その組を
   含むラウンド全体を2000mタイム昇順に並べて算出する（予選の順位を後続ラウンドへは
   引き継がない）。決勝（A決勝/B決勝/Final A〜H）は1レース＝1組で着順の焼き直しに
   なるため `(n/m)` を付けない。
...
- `(n/m)` はライブページの現時点データから計算するため、過去に手作業で作られた
  ...
  および順位の集計範囲の解釈差）。着順・タイムは取得元HTMLに忠実。
