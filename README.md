...
| 大会 | ドメイン | 文体 |
|------|----------|------|
| 関西選手権 | `karal.jp` | 1000m/2000mタイム＋着順、【N日目のレース結果】形式 |
| インカレ（全日本大学選手権） | `jara.or.jp` | 500m/2000mタイム＋着順、「種目 予選N着→ 進出先へ」サマリー形式 |
...
3. **着順の明示**: 詳細欄の行頭の番号はレーン番号なので、タイムの後ろに組内着順
   （`4着`）を付ける。過去のNURCにあった `(n/m)`（大会全体でのタイム順位）は
   ソースHTMLに無い手計算値で誤解も招きやすいため出力しない。
...
- 過去のNURCサンプルにある `(n/m)` 総合順位は出力しない（`nurc_sample/` との
  差分はこれが主因）。着順・タイムは取得元HTMLに忠実。
