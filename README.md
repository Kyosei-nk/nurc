...
| インカレ（全日本大学選手権） | `jara.or.jp` | 500m/2000mタイム、「種目 予選N着（n/m）→ 進出先へ」サマリー形式 |
...
3. **順位算出** (`nurc_gen/ranking.py`): `(n/m)` はソースHTMLに無い手計算値のため、
   各ラウンド（予選・準々決勝・準決勝・CR 等）ごとに、その組を含むラウンド全体を
   2000mタイム昇順に並べて算出する（予選の順位を後続ラウンドへは引き継がない）。
   決勝（A決勝/B決勝/Final A〜H）は1レース＝1組で組内着順の焼き直しになるため
   `(n/m)` を付けない（「E決勝2着」の表記自体が順位を表すため）。
